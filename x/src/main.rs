use chacha20poly1305::{aead::AeadInOut, legacy::ChaCha20Poly1305Legacy, KeyInit};
...
fn main() {
    for _ in 0..5000 {
        ...
        let msg_size = rand::random_range(50..2000);
        let msg = rand::random_iter().take(msg_size).collect::<Vec<u8>>();
        let ad_size = rand::random_range(50..2000);
        let ad = rand::random_iter().take(ad_size).collect::<Vec<u8>>();
